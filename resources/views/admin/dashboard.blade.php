...
@section('content')
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        ...
        <div
            class="bg-surface-container-lowest p-6 rounded-xl shadow-sm border border-outline-variant flex items-center gap-4">
        ...
        <div class="lg:col-span-2 bg-surface-container-lowest p-6 rounded-xl shadow-sm border border-outline-variant">
            <div class="flex items-center justify-between mb-6">
                ...
                <div class="flex gap-3 text-xs">
                    <span class="flex items-center gap-1"><span class="w-3 h-3 rounded-full bg-primary"></span>
                        Hilang</span>
                    <span class="flex items-center gap-1"><span class="w-3 h-3 rounded-full bg-secondary/50"></span>
                        Ditemukan</span>
                </div>
            </div>
        ...
        <div class="bg-surface-container-lowest p-6 rounded-xl shadow-sm border border-outline-variant flex flex-col">
            ...
            <div class="flex-1 flex items-center justify-center relative py-4">
                <svg class="w-48 h-48 -rotate-90" viewBox="0 0 100 100">
                    <circle cx="50" cy="50" fill="transparent" r="40" stroke="#e1e2ec" stroke-width="12">
                    </circle>
                    <circle cx="50" cy="50" fill="transparent" r="40" stroke="#0058be"
                        stroke-dasharray="251.2" stroke-dashoffset="62.8" stroke-width="12"></circle>
                    <circle cx="50" cy="50" fill="transparent" r="40" stroke="#505f76"
                        stroke-dasharray="251.2" stroke-dashoffset="188.4" stroke-width="12"></circle>
                    <circle cx="50" cy="50" fill="transparent" r="40" stroke="#ba1a1a"
                        stroke-dasharray="251.2" stroke-dashoffset="230" stroke-width="12"></circle>
                </svg>
            ...
            <div class="flex gap-2">
                ...
                <div class="relative" x-data="{ openFilter: false }">
                    <button @click="openFilter = !openFilter"
                        class="bg-surface-container-low text-on-surface-variant px-4 py-2 rounded-lg text-sm flex items-center gap-2 transition-colors duration-150 hover:bg-surface-container active:scale-95">
                        <span class="material-symbols-outlined text-sm">filter_list</span>
                        Filter
                        @if ($status)
                            <span
                                class="ml-1 px-2 py-0.5 rounded-full bg-primary/10 text-primary text-[10px] font-bold uppercase">{{ $status }}</span>
                        @endif
                    </button>

                    <div x-show="openFilter" @click.outside="openFilter = false" x-cloak
                        class="absolute right-0 mt-2 w-44 bg-surface-container-lowest rounded-lg shadow-lg border border-outline-variant py-2 text-sm z-50">
                        <a href="{{ route('admin.dashboard') }}"
                            class="block px-4 py-2 hover:bg-surface-container-low {{ !$status ? 'text-primary font-semibold' : '' }}">
                            Semua Status
                        </a>
                        @foreach (['pending', 'verified', 'claimed', 'resolved', 'rejected'] as $s)
                            <a href="{{ route('admin.dashboard', ['status' => $s]) }}"
                                class="block px-4 py-2 capitalize hover:bg-surface-container-low {{ $status === $s ? 'text-primary font-semibold' : '' }}">
                                {{ $s }}
                            </a>
                        @endforeach
                    </div>
                ...
                <tbody class="divide-y divide-outline-variant">
                    ...
                    @forelse ($recentActivities as $a)
                        <tr class="hover:bg-surface-container-low/50">
                            <td class="p-4">
                                <div class="flex items-center gap-3">
                                    <div
                                        ...
                                        <span
                                            class="material-symbols-outlined text-lg">{{ $typeIcon[$a->type] ?? 'inventory_2' }}</span>
                                    </div>
                                    <div>
                            ...
                            <td class="p-4">
                                <span
                                    class="px-3 py-1 rounded-full text-[11px] font-bold uppercase {{ $typeColor[$a->type] ?? '' }}">
                                    {{ $a->type }}
                                </span>
                            </td>
                            ...
                            <td class="p-4">
                                <span
                                    class="px-3 py-1 rounded-full text-[11px] font-bold uppercase {{ $statusColor[$a->status] ?? '' }}">
                                    {{ $a->status }}
                                </span>
                            </td>
                        </tr>
    ...
    <script>
        document.getElementById('exportBtn').addEventListener('click', function() {
            ...
            btn.disabled = true;
            btn.innerHTML =
                '<span class="material-symbols-outlined text-sm animate-spin">progress_activity</span> Exporting...';
            ...
            const head = [
                ['Barang', 'Tipe', 'Pelapor', 'Tanggal', 'Status']
            ];
            const body = [];

            document.querySelectorAll('#activityTable tbody tr').forEach(tr => {
                const cells = tr.querySelectorAll('td');
                if (cells.length < 5) return; // skip baris "Belum ada laporan"
